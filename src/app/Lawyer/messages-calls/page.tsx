...
export default function Messages() {
  ...
  // Mock message data
  const messages = [
    { id: 1, client: 'Kwame Asante', subject: 'Divorce Consultation', text: 'I need advice on divorce proceedings.', timestamp: '2025-05-23 10:00 AM', unread: true },
    { id: 2, client: 'Ama Kwarteng', subject: 'Contract Review', text: 'Can you review my employment contract?', timestamp: '2025-05-22 3:15 PM', unread: false },
    { id: 3, client: 'Kofi Mensah', subject: 'Land Dispute', text: 'I have a dispute over inherited land.', timestamp: '2025-05-21 9:30 AM', unread: false },
  ];

  // Mock call request data
  const callRequests = [
    { id: 1, client: 'Kwame Asante', time: '2025-05-23 2:00 PM', status: 'Pending' },
    { id: 2, client: 'Ama Kwarteng', time: '2025-05-24 10:00 AM', status: 'Pending' },
  ];
  ...
  const handleSendReply = () => {
    const replyTextArea = document.getElementById('reply-text') as HTMLTextAreaElement | null;
    if (replyTextArea) {
  ...
  const handleAcceptCall = (id: number) => console.log(`Accepted call request ${id}`);
  const handleRejectCall = (id: number) => console.log(`Rejected call request ${id}`);
      ...
      <div className="flex-1">
        ...
        <div className="p-6">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className={`md:col-span-2 bg-white rounded shadow p-4 ${isMobileMessageView ? 'block' : 'hidden md:block'}`}>
              {isMobileMessageView && (
                <button
              ...
              {selectedMessage ? (
                <div>
                  <h2 className="text-lg font-semibold mb-4">Message from {selectedMessage.client}</h2>
                ...
                <div className="text-gray-500 text-center">Select a message to view details.</div>
              )}

              {/* Call Requests */}
              <div className="mt-6">
                <h2 className="text-lg font-semibold mb-4 flex items-center">
                  <PhoneIcon className="h-5 w-5 mr-2 text-blue-500" /> Call Requests
                </h2>
                {callRequests.length === 0 ? (
                  ...
                  <ul className="space-y-4">
                    {callRequests.map((call) => (
                      <li key={call.id} className="p-4 border rounded flex justify-between items-center">
                        <div>
                          <p className="text-sm font-medium">{call.client}</p>
                          <p className="text-xs text-gray-500">Requested: {call.time}</p>
                        </div>
